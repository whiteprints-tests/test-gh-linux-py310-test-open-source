...
from test_gh_linux_py310_test_open_source import __version__
from test_gh_linux_py310_test_open_source.cli import APP_NAME, __app_name__
from test_gh_linux_py310_test_open_source.cli.logs import (
    LogLevel,
    configure_logging,
)
...
__all__: Final = ["test_gh_linux_py310_test_open_source"]
...
def print_copyright(ctx: Context, _param: Option, value: bool) -> None:
    ...
    console = importlib.import_module(
        "test_gh_linux_py310_test_open_source.console"
    )
...
def print_license(ctx: Context, _param: Option, value: bool) -> None:
    ...
    package_metadata = importlib.import_module(
        "test_gh_linux_py310_test_open_source.package_metadata"
    )
    console.STDOUT.print(
...
def print_debug_info(ctx: Context, _param: Option, value: bool) -> None:
    ...
    console.STDOUT.print(
        importlib.import_module(
            "test_gh_linux_py310_test_open_source.debug_info",
            __package__,
        ).gather_debug_info()
    )
    ctx.exit()
...
def test_gh_linux_py310_test_open_source(
